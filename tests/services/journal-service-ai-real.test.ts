  ...
  describe('AI availability without API key', () => {
    it('should not be available when API key is missing', async () => {
      ...
      try {
        ...
        await expect(journalService.processAIRequest('？今日はどうでしたか')).rejects.toThrow(
  ...
  describe('AI availability with API key', () => {
    it('should be available when API key is set', async () => {
      ...
      try {
        ...
        await journalService.addEntry('？今日はどうでしたか', 'AI', 'ai_question')
        await journalService.addEntry('今日は良い一日でしたね！', 'AI', 'ai_response')
